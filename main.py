  ...
  <div class="card header">
    <div class="brand">
      ...
      <div>
        ...
        <div class="brand-ver">v1.1 · IP + Domain CDN</div>
      </div>
    </div>
...
PING_SEMAPHORE = threading.Semaphore(50)

def scan_single_safe(t, port, ping_timeout, tcp_timeout):
    with PING_SEMAPHORE:

        return scan_single(t, port, ping_timeout, tcp_timeout)
...
def scan():
    ...
    def generate():
        ...
        # بهینه‌سازی ۱: تعداد تردها (workers) را به حداکثر منطقی (مثلاً ۱۰۰ تا ۲۰۰) محدود کنید 
        # تا خودِ تردها حافظه سرور را تمام نکنند.
        max_threads = min(workers, total, 150) 
        
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_threads) as executor:
            # بهینه‌سازی ۲: ارسال کارها به تابع محافظت‌شده با سمافور
            futures = {
                executor.submit(scan_single_safe, t, port, ping_timeout, tcp_timeout): t
                for t in targets
            }
